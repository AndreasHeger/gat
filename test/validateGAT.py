...
def getRegularSegmentsInAnnotations( annotations, segment_size, segment_density ):
    ...
    segments = getRegularSegments( workspace_size, segment_size, segment_density )
...
class GatTest( unittest.TestCase ):
        ...
        for key, rr in sorted(result.iteritems()):
            ...
            if not fold_is_different:
                if pvalue < self.test_threshold:
                    fail.append( "%s: expected fold change not to be different from 1.0. mean_fold=%f, pvalue=%f, folds=%s" % (key, mean_fold, pvalue, folds ) )
            else:
                if pvalue >= self.test_threshold:
                    fail.append( "%s: expected fold change to be different from 1.0. mean_fold=%f, pvalue=%f, folds=%s" % (key, mean_fold, pvalue, folds ) )
    ...
    def check( self, 
        ...
        result = self.getSamples( segments, annotations, workspace, self.nsamples )
...
class ValidationTest( GatTest ):
    ...
    def testDefault( self ):
        ...
        ss, aa, ww = self.addSingleIsochore( segmentlist, annotations, workspacelist )
    ...
    def testAscertainmentBiasFail1( self ):
        ...
        self.check( ss, aa, ww, "testAscertainmentBiasFail1", fold_is_different = True )
    ...
    def testAscertainmentBiasFail2( self ):
        ...
        self.check( ss, aa, ww, "testAscertainmentBiasFail2", fold_is_different = True )
    ...
    def testAscertainmentBiasPass( self ):
        ...
        self.check( ss, aa, ww, "testAscertainmentBiasPass", fold_is_different = False )
    ...
    def testChromosomalBiasFail( self ):
        ...
        self.check( ss, aa, ww, "testChromosomalBiasFail", fold_is_different = True )
    ...
    def testChromosomalBiasPass( self ):
        ...
        self.check( ss, aa, ww, "testChromosomalBiasPass", fold_is_different = False )
...
class SanityTest( GatTest ):
    ...
    def testOverextendingIsochoresPass( self ):
        ...
        self.check( ss, aa, ww, "testChromosomalBiasFail", fold_is_different = False )
