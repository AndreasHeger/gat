...
class TestRunning(unittest.TestCase):
    ...
    def compare(self, annotator_results):
        ...
        positions = dict([(y, x) for x, y in enumerate(
            [(x.track, x.annotation) for x in self.reference_data])])
        annotator_results.sort(key=lambda x: positions[
                               (x.track, x.annotation)])
        ...
        for attr in ("expected", "fold", "pvalue"):
            ...
            percent_differences = [
                100.0 * (x - y) / y for x, y in zip(this, ref)]
        ...
        for ref in self.reference_data:
            this = [x for x in annotator_results if x.track ==
                    ref.track and x.annotation == ref.annotation][0]
