...
def main( argv ):
    ...
    # load workspace
    workspaces = IO.readSegmentList( "workspaces", options.workspace_files, options, options.enable_split_tracks )
    ...
    E.info( "loaded %i annotations for %i genes" % (len(gene2annotations), len(annotations) ) )
    ...
    for line in IOTools.openFile( options.annotation_files[0] ) :
        ...
        indexed_genes[contig].add_interval( Interval( int(start), int(end), gene_id ) )
    ...
    for segment, segmentdict in segments.iteritems():
        ...
        for contig, ss in segmentdict.iteritems():
            for start, end in ss:
                overlapping_genes = list(indexed_genes[contig].find( start, end ))
                genes_hit_by_segments += len(overlapping_genes)
                for x in overlapping_genes:
                    gene_id = x.value
                    try:
                        for annotation in gene2annotations[gene_id]:
                            genes_hit_by_segments_with_annotations[annotation] += 1
                    except KeyError:
                        pass
